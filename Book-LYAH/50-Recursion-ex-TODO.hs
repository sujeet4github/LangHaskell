{-
    https://github.com/noelmarkham/learn-you-a-haskell-exercises/blob/master/5-recursion.hs

 -}

-- Raise x to the power y, using recursion
-- For example, power 5 2 = 25
power :: (Fractional a, Ord a, Show a) => a -> a -> a
power x y
    | y == 0    = 1
    | y > 0     = x * power x (y-1)
    | y < 0     = 1 / power x (abs y)
...
fibVal 1 = 1
fibVal x
    | x > 0     = fibVal (x-1) + (fibVal (x-2))
    | otherwise = undefined
fib :: (Num a, Eq a) => a -> [a]
fib x = map fibVal [10,9..0]
